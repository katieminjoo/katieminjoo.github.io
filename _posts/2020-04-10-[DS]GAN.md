...
Generator는 임의의 prior, $$p_z(z)$$로부터 만들어진 $$z$$를 자신이 학습하고 있는 모델, $$p_g$$의 입력값으로 받고 있습니다. 이 $$p_g$$가 원래 데이터가 가지는 확률분포, $$p_{data}$$로 근사시키는 것이 우리의 목표이기도 합니다. generator가 기존의 데이터와 서로 다른 source의 인풋을 사용하는 것 같지만, 사실 이는 $$x$$의 도메인을 무한히 확장하면(실제로 논문에서도 수렴성을 보이기 위해서 a model with infinite capacity를 가정하고 있습니다.) 결국 generator와 discriminator 모두 확률분포 함수만 다를 뿐 동일한 도메인을 사용하고 있는 것으로 해석할 수 있습니다. 마치 그림에서 보듯이 z로부터 출발을 했으나 결국 동일한 $$p_{data}$$가 가지는 도메인의 어느 영역으로 일대일 대응해줄 수 있게 되는 것입니다. 
