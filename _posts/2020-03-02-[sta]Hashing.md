...
# HashingEncoder
`HashingEncoder`도 결국 위와 같은 Hashing의 성질을 이용합니다. 따라서 아무리 많은 피쳐의 범주형 자료들을 마주하더라도, 원하는 차원의 갯수 내에서 표현이 가능해집니다. 당연히 차원의 갯수가 커질수록 정확도는 높아지고, 연산량도 증가합니다. 따라서 최적의 차원을 정하기 위한 하이퍼파라미터 튜닝과정이 필요하기는 합니다. 그럼에도 불구하고 이전에 소개한 여러 인코딩 기법들과 달리 장점도 있습니다. 이전에 인코딩 기법들은 새로운 피쳐를 마주하면 단순히 0 벡터로 값을 반환했습니다. 따라서 피쳐에 따른 결과값이 달라져도, 동일한 피쳐로 인식되기 때문에 새로운 데이터를 바탕으로한 피드백의 의미가 없게 됩니다. 그러나 HashingEncoder는 계속해서 각 피쳐에 따른 모델학습이 가능해지기 때문에 예측력을 계속해서 발전시켜나갈 수 있습니다. 물론 피쳐 갯수가 차원 수보다 엄청나게 커지면서, hashing collision이 발생하지 않는다는 가정을 하고 있습니다.    
