...
첫 번째 방식은 새로운 변수값을 마주할 때, 모두 0으로 처리를 하게끔 만듭니다. 반면 두 번째 방식은, error를 반환합니다.(만약 `inverse_tranform` 방법을 사용한다면 이 때에는 `None`값을 반환합니다.) 두 방식의 자세한 사용법은 저의 <b>[깃헙]()</b>을 참고하시면 됩니다.  
